...
3. 阅读 **`安装说明.txt`**（用文本编辑器打开，不会被 macOS 拦截）
4. **右键** `MS Fast Copy 安装助手.app` → **打开** → 弹窗中再点 **打开**
5. 在安装助手中点击 **「打开隐私与安全性设置」**，在底部点 **「仍要打开」**
6. 点击 **「启动 MS Fast Copy」**

> **为什么安装助手也会提示「无法验证」？**  
> 因为整个 zip 都未经 Apple 公证，macOS 对里面每个 `.app` 都会拦截。这不是恶意软件，是 macOS 对未签名下载的正常行为。  
> **解决办法**：右键 → 打开（只需一次），或阅读 `安装说明.txt` 里的终端命令。
